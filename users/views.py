...
from .serializers import LoginSerializer,UserRegisterSerializer,PasswordResetSerializer
...
class SendOtpForgotView(APIView):
    def post(self, request):
        ...
        if User.objects.filter(phone_number=str(phone_number)).exists():
            ...
            return Response({'error': 'This number is Not registered'}, status=status.HTTP_400_BAD_REQUEST)
...
class VerifyOtpView(APIView):
    def post(self, request):
        ...
        if not stored_otp or not otp_send_time:
            return Response({'error': 'OTP data is incomplete or missing'}, status=status.HTTP_400_BAD_REQUEST)
