...
class OTPTestCase(TestCase):
    def setUp(self):
        ...
        self.send_request = self.factory.post(
            "/api/user/send-otp", {"phone_number": self.phone_number}
        )
        ...
        self.otp = self.send_request.session[
            "otp"
        ]  # Save the generated OTP for later use

    def test_verify_otp(self):
        # Set up VerifyOtpView request with the correct OTP
        verify_request = self.factory.post(
            "/api/user/verify-otp", {"otp": str(self.otp)}
        )
        verify_request.session = (
            self.send_request.session
        )  # Share session with the verify request
