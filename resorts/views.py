...
from .serializers import ResortSerializer, ResortImageSerializer,UserResortSerializer,CategorySerializer
...
class ResortView(APIView):
    def get(self, request):
        resorts = Resort.objects.prefetch_related('images').all()
        serializer = UserResortSerializer(resorts, many=True)
        return Response(serializer.data)
    
class ResortDetailView(APIView):
    # permission_classes = [IsAuthenticayyted]

    def get(self, request, pk):
        print('pk',pk)  
        try:
            resort = Resort.objects.get(id=pk, valid=True)  # Ensure valid resort
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if result.get("result") == "ok":
                ...
                return JsonResponse({"error": "Failed to delete image", "details": result}, status=400)
        except Exception as e:
            return JsonResponse({"error": "Cloudinary error", "details": str(e)}, status=500)
