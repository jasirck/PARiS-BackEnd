...
class BookFlightView(APIView):
    ...
    def post(self, request):
        try:
            ...
            if not flight_serializer.is_valid():
                return Response(
                    flight_serializer.errors, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            booked_flight_serializer = BookedFlightSerializer(
                data=transformed_booking_data
            )
            if not booked_flight_serializer.is_valid():
                flight.delete()
                return Response(
                    booked_flight_serializer.errors, status=status.HTTP_400_BAD_REQUEST
                )
    ...
    def get(self, request, *args, **kwargs):
        ...
        booked_flights = BookedFlight.objects.filter(user=request.user).order_by(
            "-created_at"
        )
