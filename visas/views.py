...
from .serializers import VisaCategorySerializer, VisaSerializer, VisaDaysSerializer,GetVisaSerializer,VisaBookedSerializer,AdminBookedVisaSerializer
        ...
        if visa_serializer.is_valid():
            ...
            for visa_day_data in visa_days_data:
                ...
                if visa_day_serializer.is_valid():
                    ...
                    print(f"Visa day {visa_day_data} failed validation: {visa_day_serializer.errors}")
                    return Response(visa_day_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class VisaDetailView(APIView):
    ...
    def put(self, request, pk):
        ...
        visa_serializer = VisaSerializer(visa, data=visa_data, partial=True)  # Use partial=True to allow partial updates
        if visa_serializer.is_valid():
...
class BookedVisaView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(AdminBookedVisaSerializer(booked_visa).data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
