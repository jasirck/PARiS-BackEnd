...
class VisaBookedSerializer(serializers.ModelSerializer):
    ...
    booked_visa_place_photo = serializers.CharField(source='booked_visa.place_photo', read_only=True)
    ...
    price = serializers.DecimalField(source='days.price', max_digits=10, decimal_places=2)
    ...
    class Meta:
        model = VisaBooked
        fields = ['id', 'user', 'days', 'passport', 'photo','conformed', 'booked_visa', 'booked_visa_name', 'booked_visa_place_photo','created_at','user_name','day','price','number','email']



class AdminBookedVisaSerializer(serializers.ModelSerializer):
    user = serializers.PrimaryKeyRelatedField(queryset=User.objects.all())  # Instead of user_id
    booked_visa = serializers.PrimaryKeyRelatedField(queryset=Visa.objects.all())
    days = serializers.PrimaryKeyRelatedField(queryset=VisaDays.objects.all())  # Use days instead of days_id
