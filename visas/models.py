...
class Visa(models.Model):
    ...
    category = models.ForeignKey(
        "VisaCategory", on_delete=models.CASCADE, related_name="visa_category"
    )
...
class VisaDays(models.Model):
    ...
    price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Visa Price"
    )
...
class VisaBooked(models.Model):
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="visa_bookings"
    )
    days = models.ForeignKey(
        VisaDays, on_delete=models.CASCADE, related_name="visa_days"
    )
    passport = models.TextField(verbose_name="Passport Photo")
    photo = models.TextField(verbose_name="Personal Photo")
    booked_visa = models.ForeignKey(
        Visa, on_delete=models.CASCADE, related_name="bookings"
    )
