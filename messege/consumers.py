...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        try:
            ...
            if is_admin:
                self.admin = await database_sync_to_async(Admin.objects.get)(
        ...
        if self.user:
            ...
            await self.send(
                text_data=json.dumps(
                    {"type": "contact_list", "contacts": active_sessions}
                )
            )
    ...
    async def handle_message(self, data):
        ...
        if self.user:
            ...
            chat_message = await self.save_message(
                self.user.id, None, message, is_admin=False
            )
            message_data = {
                "type": "chat_message",
                "message": message,
            ...
            chat_message = await self.save_message(
                self.admin.id, receiver_id, message, is_admin=True
            )
            message_data = {
                "type": "chat_message",
                "message": message,
            ...
            receiver_group_name = (
                f"chat_user_{receiver_id}"  # Send to the specific user
            )

        # Send the message to the receiver's group and the sender's group
        await self.channel_layer.group_send(
            receiver_group_name, {"type": "chat_message", **message_data}
        )
        await self.channel_layer.group_send(
    ...
    async def handle_fetch_history(self, data):
        ...
        if user_id:
            ...
            await self.send(
                text_data=json.dumps({"type": "chat_history", "messages": chat_history})
            )
    ...
    async def handle_typing_status(self, is_typing):
        if self.user:
            ...
            await self.channel_layer.group_send(
                f"chat_user_{self.current_chat_user_id}",
                {"type": "user_typing", "is_typing": is_typing},
            )

    async def chat_message(self, event):
        # Remove type key as it's used by the channel layer
        message_data = {k: v for k, v in event.items() if k != "type"}
        await self.send(text_data=json.dumps({"type": "chat_message", **message_data}))
    ...
    def mark_messages_as_read_by_user(self, user_id):
        ChatMessage.objects.filter(sender_user_id=user_id, is_read=False).update(
            is_read=True
        )

    @database_sync_to_async
    def get_active_chat_sessions(self):
        sessions = ChatSession.objects.select_related("user").prefetch_related(
            "messages"
        )
        active_sessions = []
        ...
        for session in sessions:
            ...
            unread_count = session.messages.filter(
                is_read=False, sender_user=session.user
            ).count()
    ...
    def get_chat_history(self, user_id):
        messages = (
            ...
            .select_related("sender_admin", "sender_user")
            .order_by("timestamp")
        )
        ...
        return [
            {
                ...
                "message": msg.message,
                "sender_id": (
                    msg.sender_admin.id if msg.sender_admin else msg.sender_user.id
                ),
