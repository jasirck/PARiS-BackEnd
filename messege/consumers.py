...
#         messsage = ChatMessage.objects.filter(sender_user=message_ids)
#         ChatMessage.objects.filter(sender_user=message_ids, is_read=False).update(is_read=True)
#         # ChatMessage.objects.filter(id__in=message_ids, is_read=False)
...
#             unread_count = session.messages.filter(is_read=False).count()
...
#             sesion, _ = ChatSession.objects.get_or_create(user=receiver_user)
...
#             sesion, _ = ChatSession.objects.get_or_create(user=sender_user)
#             return ChatMessage.objects.create(
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_typing_status(self, is_typing):
        if self.user:
            ...
            # Send admin typing status to specific user
