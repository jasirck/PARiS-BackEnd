...
class ChatMessage(models.Model):
    ...
    def __str__(self):
        if self.sender_user:
            return f"From User {self.sender_user.username} to Admins: {self.message[:20]}"
        else:
    ...
    def clean(self):
        if not (self.sender_user or self.sender_admin):
            raise ValidationError("A message must have a sender (either user or admin).")
        if self.sender_user and self.sender_admin:
            raise ValidationError("A message cannot have both a user and an admin as the sender.")
        ...
        if self.sender_user and self.receiver_user:
            raise ValidationError("A user's message cannot have a receiver_user (it goes to all admins).")
